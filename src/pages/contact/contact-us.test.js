...
describe("DemoRequestPage", () => {
  ...
  test("validates input fields and enables submit button when all fields are valid", async () => {
    ...
    expect(
      screen.queryByText("Please enter a valid mobile number.")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Please enter a valid company name.")
    ).not.toBeInTheDocument();
  ...
  test("shows error messages for invalid inputs", () => {
    ...
    expect(
      screen.getByText("Please enter a valid company name.")
    ).toBeInTheDocument();
  });
});
